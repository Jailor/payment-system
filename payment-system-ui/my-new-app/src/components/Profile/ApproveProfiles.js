...
function ApproveProfiles() {
  ...
  const REDIRECT = "http://localhost:3000/redirect/view-profiles";
  ...
  return (
    <div className="container">
      {["CREATE", "REMOVE"].includes(profileData.operation.name) && (
        <div>
          <h2>Profile Information for {profileData.operation.name}</h2>
          <p>
            <strong>Profile name:</strong> {profileData.name}
          </p>
          <p>
            <strong>Profile type:</strong> {profileData.profileType.name}
          </p>
          <p>
            <strong>Rights:</strong> {profileData.rights.join(", ")}
          </p>
          <p>
            <strong>Status:</strong> {profileData.status.name}
          </p>
          ...
          <div>
            <button onClick={handleApprove} className="button">
              Approve {profileData.operation.name}
            </button>
            <button onClick={handleReject} className="button">
              Reject {profileData.operation.name}
            </button>
          </div>
        </div>
      )}

      {profileData.operation.name === "MODIFY" && (
        <div>
          ...
          <div className="row">
            ...
            <div className="column">
              ...
              <p>
                <strong>Profile type:</strong> {profileData.newProfileType.name}
              </p>
              <p>
                <strong>Rights:</strong> {profileData.newRights.join(", ")}
              </p>
              <p>
                <strong>Status:</strong> {profileData.newStatus.name}
              </p>
            </div>
          </div>
          <Alert color="danger" isOpen={show} toggle={onDismiss}>
