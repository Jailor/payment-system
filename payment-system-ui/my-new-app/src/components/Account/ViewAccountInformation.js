...
function ViewAccountInformation(props) {
  ...
  const handleDelete = async () => {
    try {
      ...
      window.location.href = "http://localhost:3000/redirect/view-accounts"
    } catch (error) {
  ...
  const REDIRECT = "http://localhost:3000/redirect/view-accounts";
  ...
  return (
    <div className="container">
      ...
      <p>
        <strong>Account Status:</strong> {accountObject.accountStatus.name}
      </p>
      <p>
      ...
      <div className="action-buttons">
        ...
        <button
          ...
          disabled={
            accountObject.status.name === "CLOSED" ||
            accountObject.status.name === "REMOVED" ||
            accountObject.accountStatus.name === "CLOSED" ||
            accountObject.accountStatus.name === "BLOCKED" || 
            accountObject.accountStatus.name === "BLOCKED_DEBIT"
          }
        >
          Transaction
        </button>
    ...
    <ul>
        {approvalPayments && approvalPayments.map((payment, index) => (
          <ul key={index} className={`info ${payment.status.name === "BLOCKED_BY_FRAUD" ? "fraud-payment" : ""}`}>
            <div>
              ...
              {payment.status.name === "VERIFY" && sessionStorage.getItem("rights").includes("VERIFY_PAYMENT") &&(
                <button
                type="button"
                className="button"
                onClick={() => {
                ...
                {payment.status.name === "AUTHORIZE" && sessionStorage.getItem("rights").includes("AUTHORIZE_PAYMENT") &&(
                <button
                type="button"
                className="button"
                onClick={
                ...
                {payment.status.name === "APPROVE" && sessionStorage.getItem("rights").includes("APPROVE_PAYMENT") &&(
                <button
                type="button"
                className="button"
                onClick={
                ...
                {payment.status.name === "BLOCKED_BY_FRAUD" && sessionStorage.getItem("rights").includes("APPROVE_PAYMENT") &&(
                <button
                type="button"
                className="button"
                onClick={
              ...
              {(
               payment.status.name === "VERIFY" || 
               payment.status.name === "AUTHORIZE"||
               payment.status.name === "APPROVE" || 
               payment.status.name === "BLOCKED_BY_FRAUD")
              && (
                <button
                type="button"
                className="button"
