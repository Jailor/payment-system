...
function TransactionModalGeneral({ isOpen, onRequestClose }) {
    ...
    const transferData = {
      ...
      try {
        ...
        if (!handleErrors(response.data.errors)) {
          ...
          window.location.href = "/redirect/approve-payments";
        }
      } catch (error) {
  ...
  return (
    ...
    >
      <h2>Transaction</h2>
      <form>
        <label>Credit Account Number (TO):</label>
        <input
          type="text"
        ...
        <label>Debit Account Number (FROM):</label>
        <input
          type="text"
...
export default TransactionModalGeneral;
